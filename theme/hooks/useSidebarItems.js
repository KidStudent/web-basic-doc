...
export const headersToSidebarItemChildren = (headers, sidebarDepth) =>
  sidebarDepth > 0 ? headers.map((header) => headerToSidebarItem(header, sidebarDepth - 1)) : [];
/**
...
export const resolveArraySidebarItems = (sidebarConfig, sidebarDepth) => {
  ...
  const handleChildItem = (item) => {
    ...
    if (childItem.link === route.path) {
      // skip h1 header
      const headers =
        page.value.headers[0]?.level === 1 ? page.value.headers[0].children : page.value.headers;
      return {
        ...childItem,
