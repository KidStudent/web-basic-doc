...
import { SourceFactory } from './factory/source.factory';
import { LayerFactory } from './factory/layer.factory';
import { EditFactory } from './factory/edit.factory';
import { FenceFactory } from './factory/fence.factory';

export class mapMain {
  map = null;
  mapDomId = null; /*地图mapId 必填*/
  mapStyle = null; /*球面矢量底图样式 必填*/
  mapCenter = null; /*地图中心点 必填*/
  mapZoom = null; /*地图默认缩放等级 必填*/
  mapPitch = 0; /*地图俯仰角度*/
  mapBearing = 0; /*地图旋转角度*/
  minZoom = 1; /*地图最小缩放等级*/
  maxZoom = 22; /*地图最大缩放等级*/
  mapProjection = 'MERCATOR'; /*地图坐标系*/

  ImageFactory; /*加载图片工厂类*/
  LayerFactory; /*图层工厂类*/
  EditFactory; /*图形绘制工厂类*/
  FenceFactory; /*围栏工厂类*/
  rangingTool; /*测距以及测面积工具*/

  markers = []; /*地图上所有单个点位集合*/
  popups = []; /*地图上所有弹框集合*/
  sources = []; /*地图上所有根源点集合*/
  layers = []; /*地图上所有图层集合*/
  primitives = []; /*地图上所有primitive集合*/
