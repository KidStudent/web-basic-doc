...
// createNamespace-button__element--disable
...
function createBEM(prefixName: string) {
  ...
  const bm = (blockName, modifierName) =>
    blockName && modifierName ? _bem(prefixName, blockName, '', modifierName) : '';
  const em = (elementName, modifierName) =>
    elementName && modifierName ? _bem(prefixName, '', elementName, modifierName) : '';
